...
class SetupIrcCommand: ChatCommand("setupirc", "Sets up IRC access for your account") {
    override fun onCommand(context: CommandContext, command: String, args: List<String>): Boolean {
        if (context.sender is User) {
            context.sender.sendPrivateMessage(Hirasawa.banchoBot, "Generating new IRC token")
            ...
            context.sender.sendPrivateMessage(Hirasawa.banchoBot, "Your IRC token is $token")
            context.sender.sendPrivateMessage(Hirasawa.banchoBot, "Use this as your password when connecting on IRC")
            context.sender.sendPrivateMessage(Hirasawa.banchoBot, "Run !setupirc again to generate a new token")
        }
